...
//Crear producto.
router.post('/', middle_auth.validate, middle_auth.verify_permisson, async function (req, res, next) {
    let data = req.body;
    let files = req.files;
...
router.put('/quantity', middle_auth.validate, middle_auth.verify_permisson, async function (req, res, next) {
    let data = req.body;
    res.json(await product_ctr.update_quantity(data));
});

//Eliminar producto.
router.delete('/:product_id', middle_auth.validate, middle_auth.verify_permisson, async function (req, res, next) {
